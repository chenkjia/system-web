...
3. 如何使用应用中开发好的组件，查看【<a href="#howToWriteDemo">下文</a>】，请开发能共同使用方便查看的文档~
4. 以下行为都需要重新npm run docs:dev （如果有发现能解决该问题的分享一下~）
    - 任何修改/.vuepress/config.js 
    - 任何增加、删除、重命名文件夹/文件
    - 修改.md文件的层级关系等触发侧边栏层级关系的刷新
5. 在代码上传，且文档同步完成后，推送到**release-docs分支**，文档自动化部署后，可在以下环境在线预览
    - [内网：192.168.0.238:8090](192.168.0.238:8090)
    - [外网：http://fedev.gddxit.cn:8021/](http://fedev.gddxit.cn:8021/)
...
### <a name="howToWriteDemo">使用src/component中已经开发好的组件写demo</a>
修改组件引入方式，组件注册只需在/src/components/index.js全局注册即可全部使用
1）在项目中开发好组件，/src/components/index.js全局注册的组件，无需再在.vuepress/enhanceApp.js中注册，可直接应用引用。非全局组件需要在enhanceApp.js中再单独引用。在.vuepress/components文件目录下写入对应的示例demo，官方已说明``docs/.vuepress/components: 该目录中的 Vue 组件将会被自动注册为全局组件。``[详细目录结构查看](https://vuepress.vuejs.org/zh/guide/directory-structure.html)
2）写demo
...
懒癌朋友可以关注【<a href="#howToCreateActiDemo">如何自动化将demo生成md</a>】
...
gitlab pages 默认是关闭的，如何开启gitlab pages服务，由于种种问题，我们最终选择搭配jenkins自动化部署到 [在线预览文档系统](http://fedev.gddxit.cn:8021/)
...
### <a name="howToCreateActiDemo">如何自动化将demo生成md</a>
TODO
